...
const createNewShipping = async (data) => {

    try {

        // if (data.name && data.image && data.price && data.priceOld && data.status && data.shippingId) {
        //     return {
...
const updateShipping = async (data) => {
    try {
        ...
        //     }
        // }
        let blog = await db.Shipping.findOne({
            where: { id: data.id }
        })
        if (blog) {
            await blog.update({
...
module.exports = {
    updateShipping, createNewShipping, deleteShipping
}